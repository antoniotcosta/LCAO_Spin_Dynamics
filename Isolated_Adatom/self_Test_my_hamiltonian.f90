...
MODULE the_hamiltonian
 ...
 COMPLEX(double), DIMENSION(dimG,dimG) :: Hllc,Hrlc,H01_central,H10_central
 COMPLEX(double), DIMENSION(dimG,dimG) :: Hcll,Hcrl
 COMPLEX(double), DIMENSION(dimG,dimG) :: mum_l,tl,tld
 COMPLEX(double), DIMENSION(dimG,dimG) :: mum,H00
...
SUBROUTINE fcn(NX,X,fvec,iflag)
 ...
 REAL(double) :: n_up_Bi,n_dn_Bi,n_T_Bi
  ...
  DO i=1,Nadatoms
  WRITE(*,"('------hdel(',I0,')------')")i
  WRITE(*,"(2(2(f15.5,1x),5x))")REAL(hdel_A(i,1,1)),AIMAG(hdel_A(i,1,1)),&
                                REAL(hdel_A(i,1,2)),AIMAG(hdel_A(i,1,2))
  WRITE(*,"(2(2(f15.5,1x),5x))")REAL(hdel_A(i,2,1)),AIMAG(hdel_A(i,2,1)),&
                                REAL(hdel_A(i,2,2)),AIMAG(hdel_A(i,2,2))
  END DO
  END IF
  ...
  IF (myid==0) THEN
   ...
   WRITE(*,"('m_f(',I0,')=',3(e24.13,1x))")i,m_f(:,i)
   END DO
   WRITE(*,*)
   WRITE(*,*)
   WRITE(*,*)
