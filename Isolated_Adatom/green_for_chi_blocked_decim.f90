SUBROUTINE green(wc,gout_ad)
 ...
 COMPLEX(double), DIMENSION(4*dimH_p3,4*dimH_p3) :: g0l_blk,gll_blk,glr_blk,glb_blk
 COMPLEX(double), DIMENSION(4*dimH_p3,4*dimH_p3) :: tl_blk,tld_blk
 ...
 g0l_blk(1:2*dimH_p3,1:2*dimH_p3) = g0_l
 g0l_blk(2*dimH_p3+1:4*dimH_p3,2*dimH_p3+1:4*dimH_p3) = g0_l
 g0l_blk(1:2*dimH_p3,2*dimH_p3+1:4*dimH_p3) = -tl
 g0l_blk(2*dimH_p3+1:4*dimH_p3,1:2*dimH_p3) = -tld

 tl_blk = CMPLX(0d0,0d0,double)
 tld_blk = CMPLX(0d0,0d0,double)
 tl_blk(2*dimH_p3+1:4*dimH_p3,1:2*dimH_p3) = tl
 tld_blk(1:2*dimH_p3,2*dimH_p3+1:4*dimH_p3) = tld

 CALL decim(g0l_blk,tl_blk,tld_blk,gll_blk,glr_blk,glb_blk,cri,dimdecim,itmax)

 CALL invers(gll_blk,dimdecim)
 CALL invers(glr_blk,dimdecim)

 gl_l = gll_blk(1:2*dimH_p3,1:2*dimH_p3)
 gl_r = glr_blk(2*dimH_p3+1:4*dimH_p3,2*dimH_p3+1:4*dimH_p3)
