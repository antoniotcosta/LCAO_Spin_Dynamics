...
MODULE  lattice
 ...
 INTEGER, PARAMETER :: N_Bi = 24, Nadatoms = 1, N_uc = N_Bi+NAdatoms+6 
...
MODULE the_hamiltonian
 ...
 COMPLEX(double), DIMENSION(dimG,dimG) :: tl,tld
...
SUBROUTINE fcn(NX,X,fvec,iflag)
 ...
 INTEGER :: il,ix,is
  ...
  IF (myid==0) THEN
   WRITE(*,"('--------output----------')")
   DO is=1,Nadtotal
   WRITE(*,"('n_d(',I0,') = ',e18.6)")is,SUM(n_orb_u(is,5:9)+n_orb_d(is,5:9))
   WRITE(*,"('n_T(',I0,') = ',e18.8)")is,n_t(is)
   WRITE(*,"('m_f(',I0,') = ',3(e15.5,1x))")is,m_f(:,is)
   WRITE(*,"('fvec(1) = ',4(e15.5,1x))")fvec(is),fvec(Nadtotal+(is-1)*3+1:Nadtotal+(is-1)*3+3)
   WRITE(*,*)
   END DO
  END IF
