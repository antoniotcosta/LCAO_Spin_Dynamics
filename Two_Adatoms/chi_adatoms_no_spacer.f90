PROGRAM chi_single_TM_adatom_on_TI
 ...
 COMPLEX(double), DIMENSION(4*25*Nadtotal,25*Nadtotal) :: chi0,chi
 ...
 REAL(double) :: ImTrchi
 COMPLEX(double) :: y
 COMPLEX(double), DIMENSION(4,Nadtotal,Nadtotal) :: sum_chi_RPA, sum_chi0


 REAL(double), DIMENSION(3,Nadtotal) :: m_i

 INTEGER :: ierr,ifile
 INTEGER :: mu,nu,l,m,n,is,js
 ...
 CHARACTER(len=255) :: nome
  ...
  DO iw = 1,nw
   ...
   DO nu=1,5
    ...
    n = (js-1)*25 + 5*(nu-1) + nu
    sum_chi_RPA(l,is,js) = sum_chi_RPA(l,is,js) + chi(m,n) 
    sum_chi0(l,is,js) = sum_chi0(l,is,js) + chi0(m,n)
   END DO
   END DO
   END DO
   END DO
  ...
  DO is=1,Nadtotal
  DO js=1,Nadtotal
  WRITE(nome,"('chi_',I0,'_',I0,'_',I0,'.dat')")l,is,js
  OPEN(unit=999,file=nome,status="unknown",position="append")
  WRITE(999,*)e,REAL(sum_chi_RPA(l,is,js)),aimag(sum_chi_RPA(l,is,js))
  CLOSE(999)
  END DO
  END DO
  END DO
  ...
  DO is=1,Nadtotal
  DO js=1,Nadtotal
  WRITE(nome,"('chi0_',I0,'_',I0,'_',I0,'.dat')")l,is,js
  OPEN(unit=999,file=nome,status="unknown",position="append")
  WRITE(999,*)e,REAL(sum_chi0(l,is,js)),aimag(sum_chi0(l,is,js))
  CLOSE(999)
  END DO
  END DO
  END DO
